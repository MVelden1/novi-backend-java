...
## Inleiding
Je gaat een class schrijven die we kunnen gebruiken om oneindig veel Auto-instanties te kunnen creëren. Zo kun je een mooie start maken met het aanmaken van classes, instanties en alle bijbehorende logica. Bron: https://github.com/hogeschoolnovi/backend-java-car-class (uitwerkingen staan daar op de branch _uitwerkingen_).

## Voor je begint

Deze opdracht is overgenomen uit de NOVI-repo en ingepast in deze lesrepo. Wat
er anders is dan in het origineel:

- geen Maven, dus geen `pom.xml` en geen `mvnw`. Alles draait via het
  IntelliJ-build system, net als de rest van deze repo
- geen aparte `src/main/java` en `src/test/java`. Code en tests staan hier in
  dezelfde map, in package `nl.novi.les2.voorbereiding.carclass`
- elke class heeft daarom bovenaan een `package`-regel. Nieuwe classes die je
  hier aanmaakt krijgen die van IntelliJ automatisch

Eenmalig instellen: de tests gebruiken JUnit 5, dat zit nog niet in deze
module. Open `MainTest.java`, zet je cursor op de rode `org.junit`-import en
druk `Alt+Enter`, kies *Add 'JUnit5' to classpath*. IntelliJ haalt de library
dan zelf op.

Let op: `StappenTest` en `MainTest` verwijzen naar een class `Car` die nog niet
bestaat. Zolang die er niet is, compileert de module niet. Maak `Car.java` dus
als eerste aan, ook al is hij nog leeg.

### 🟩 Code runnen
Groene pijl in de kantlijn naast `main`, of `Ctrl+Shift+F10`.

### 🟨 Testen uitvoeren
Groene pijl in de kantlijn naast de testklasse of naast een losse testmethode.
Het stappenplan hieronder vertelt je steeds welke test je op dat moment moet
draaien.
...
1. `Main.java` staat er al. Controleer of hij bovenaan de juiste `package`-regel heeft.
2. Zet in de Main class de methode `public static void main(String[] args)`.
3. Maak nog een class aan in deze map. Noem deze class `Car` en zet er één property in voor het merk van de auto. Welk type moet deze variabele krijgen?
   _Tip_: declareer de variabele in het Engels.
4. Probeer eens uit of je een (lege) instantie kunt maken van jouw pas-gemaakte class. Dit doe je natuurlijk in het bestand `Main.java`. Print het merk-property van deze instantie in de terminal om te controleren of deze echt leeg is!  _Tip:_ System.out.println(instantie.property)
